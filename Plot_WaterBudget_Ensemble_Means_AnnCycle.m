...
for c=1:3
    ...
    set(gca,'fontsize',32)
    ylabel('ET (mm/day)','fontsize',32)
    xlim([min(plot_x) max(plot_x)])
    xlabel('day of water year','fontsize',32)
    if c==1
        leg = legend([p01 p02 p03 p04 p05],{'Baseline','Mod-params','Mod-params+GVF','Mod-params+GVF+Veg-class','Mod-params+GVF+Veg-class+snow-alb'},'fontsize',23,'location','best');
    end
    ...
    box on
    set(gca,'fontsize',32)
    ylabel('\DeltaET relative to baseline (mm/day)','fontsize',32)
    xlim([min(plot_x) max(plot_x)])
    xlabel('day of water year','fontsize',32)
    ...
    set(gca,'fontsize',32)
    ylabel('ET paritions (mm/day)','fontsize',32)
    xlim([min(plot_x) max(plot_x)])
    xlabel('day of water year','fontsize',32)
    if c==1
        leg = legend([p01 p02 p03 p011 p022 p033],{'Baseline E_{tran}','Baseline E_{can}','Baseline E_{dir}','Mod-params+GVF+Veg-class+snow-alb E_{tran}','Mod-params+GVF+Veg-class+snow-alb E_{can}','Mod-params+GVF+Veg-class+snow-alb E_{dir}'},'fontsize',23,'location','best');
    ...
    box on
    set(gca,'fontsize',32)
    ylabel({'\DeltaET paritions'; 'relative to baseline (mm/day)'},'fontsize',32)
    xlim([min(plot_x) max(plot_x)])
    xlabel('day of water year','fontsize',32)
    saveas(f,sprintf('/Users/abolafia/ASO_Fire/Plots/PaperPlots/ET_paritions_ensemble_AnnCycle_%s.png',catch_names{c}))
    ...
    xlabel('day of water year','fontsize',32)
    if c==1
        leg = legend([p01 p02 p03 p011 p022 p033],{'mod-params+GVF E_{tran}','mod-params+GVF E_{can}','mod-params+GVF E_{dir}','Mod-params+GVF+Veg-class E_{tran}','Mod-params+GVF+Veg-class E_{can}','Mod-params+GVF+Veg-class E_{dir}'},'fontsize',23,'location','best');
        leg.Position = [0.1371    0.7680    0.3562    0.1784];
    end
    f.Position = [-1919        -299        1165        1096];
    ...
    box on
    set(gca,'fontsize',32)
    ylabel({'\DeltaET paritions';'Mod-params+GVF+Veg-class - mod-params+GVF'; '(mm/day)'},'fontsize',20)
    xlim([min(plot_x) max(plot_x)])
    xlabel('day of water year','fontsize',32)
    saveas(f,sprintf('/Users/abolafia/ASO_Fire/Plots/PaperPlots/ET_paritions_ensemble_AnnCycle_%s_VegClassImpact.png',catch_names{c}))
    ...
    box on
    set(gca,'fontsize',32)
    ylabel({'\DeltaET paritions';'Mod-params+GVF+Veg-class+snow-alb - mod-params+GVF+Veg-class'; '(mm/day)'},'fontsize',20)
    xlim([min(plot_x) max(plot_x)])
    xlabel('day of water year','fontsize',32)
    saveas(f,sprintf('/Users/abolafia/ASO_Fire/Plots/PaperPlots/ET_paritions_ensemble_AnnCycle_%s_AlbedoImpact.png',catch_names{c}))
    ...
    set(gca,'fontsize',32)
    ylabel('SWE (mm)','fontsize',32)
    xlim([min(plot_x) max(plot_x)])
    xlabel('day of water year','fontsize',32)
    if c==1
        leg = legend([p01 p02 p03 p04 p05],{'Baseline','Mod-params','Mod-params+GVF','Mod-params+GVF+Veg-class','Mod-params+GVF+Veg-class+snow-alb'},'fontsize',23,'location','best');
        leg.Position = [0.5221    0.7915    0.3918    0.1374];
    end
    f.Position = [-1919        -299        1165        1096];
    ...
    set(gca,'fontsize',32)
    ylabel({'\Delta SWE';'relative to baseline (mm)'},'fontsize',32)
    xlim([min(plot_x) max(plot_x)])
    xlabel('day of water year','fontsize',32)
    ...
    box on
    set(gca,'fontsize',32)
    ylabel('total column SM (mm/mm)','fontsize',32)
    xlim([min(plot_x) max(plot_x)])
    xlabel('day of water year','fontsize',32)
    if c==1
        leg = legend([p01 p02 p03 p04 p05],{'Baseline','Mod-params','Mod-params+GVF','Mod-params+GVF+Veg-class','Mod-params+GVF+Veg-class+snow-alb'},'fontsize',23,'location','best');
    ...
    set(gca,'fontsize',32)
    ylabel({'\Delta SM';'relative to baseline (mm/mm)'},'fontsize',32)
    xlim([min(plot_x) max(plot_x)])
    xlabel('day of water year','fontsize',32)
    ...
    sprintf('mod-params+gvf+class+alb results in %.4f%% change in SM at %s',mean((annual_SM_realistic - annual_SM_baseline)./annual_SM_baseline) * 100,catch_names{c})
    f=figure;
    subplot(2,1,1)
    hold on
    ...
    box on
    set(gca,'fontsize',32)
    ylabel('subsurface flow (mm/day)','fontsize',32)
    xlim([min(plot_x) max(plot_x)])
    xlabel('day of water year','fontsize',32)
    if c==1
        leg = legend([p01 p02 p03 p04 p05],{'Baseline','Mod-params','Mod-params+GVF','Mod-params+GVF+Veg-class','Mod-params+GVF+Veg-class+snow-alb'},'fontsize',23,'location','best');
        leg.Position = [0.5719    0.8110    0.3918    0.1286];
    end
    ...
    set(gca,'fontsize',32)
    ylabel({'\Delta subsurface flow';'relative to baseline (mm)'},'fontsize',32)
    xlim([min(plot_x) max(plot_x)])
    xlabel('day of water year','fontsize',32)
