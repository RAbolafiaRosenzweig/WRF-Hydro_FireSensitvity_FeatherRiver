...
post_fire_start = datenum([2021 10 1]);
pre_fire_end = datenum([2019 9 1]);
...
saveas(f,'/Users/abolafia/ASO_Fire/Plots/PaperPlots/Baseline_and_Realistic_EBNFF_Qanom_scatter_Standardized.png')
...
saveas(f,'/Users/abolafia/ASO_Fire/Plots/PaperPlots/Baseline_and_Realistic_EBNFF_Bias_Boxplot_Standardized.png')
...
% end
xtickangle(35)
saveas(f,'/Users/abolafia/ASO_Fire/Plots/PaperPlots/Baseline_EBNFF_Qanom_timeseries_prefire_Standardized.png')
...
% if p>0.001
%     text(dates_postfire(125),1.5,sprintf('R^{2}=%.2f(p=%.2f);NSE =%.2f;bias=%.2f',round(R_baseline^2,2),round(p_baseline,2),round(NSE_baseline,2),round(bias_baseline,2)),'fontsize',40,'color','b')
% else
%     text(dates_postfire(125),1.5,sprintf('R^{2}=%.2f(p<0.001);NSE =%.2f;bias=%.2f',round(R_baseline^2,2),round(NSE_baseline,2),round(bias_baseline,2)),'fontsize',40,'color','b')
% end
...
% % if p>0.001
% %     text(dates_postfire(125),1,sprintf('R^{2}=%.2f(p=%.2f);NSE =%.2f;bias=%.2f',round(R_realistic^2,2),round(p_realistic,2),round(NSE_realistic,2),round(bias_realistic,2)),'fontsize',40,'color','r')
% % else
% %     text(dates_postfire(125),1,sprintf('R^{2}=%.2f(p<0.001);NSE =%.2f;bias=%.2f',round(R_realistic^2,2),round(NSE_realistic,2),round(bias_realistic,2)),'fontsize',40,'color','r')
% % end
xtickangle(35)
ylim([-1 3.5])
saveas(f,'/Users/abolafia/ASO_Fire/Plots/PaperPlots/EBNFF_Qanom_timeseries_postfire_Standardized.png')
...
idx_low_flow = find(store_stats(:,1) < 0 & store_stats(:,5) > 2 & store_stats(:,5) < 4);
...
for k=1:length(idx_low_flow)
    ...
    text(current_dates(10),4.2,sprintf('R^{2}=%.2f; NSE = %.2f; bias = %.2f',round(R^2,2),round(NSE,2),round(bias,2)),'fontsize',30,'color','b')
    % if p>0.001
    %     text(dates_prefire(1000),10,sprintf('R^{2}=%.2f (p=%.2f); NSE = %.2f; bias - %.2f',round(R^2,2),round(p,2),round(NSE,2),round(bias,2)),'fontsize',40,'color','b')
    % else
    %     text(dates_prefire(1000),10,sprintf('R^{2}=%.2f (p<0.001); NSE = %.2f; bias = %.2f',round(R^2,2),round(NSE,2),round(bias,2)),'fontsize',40,'color','b')
    % end
    xtickangle(35)
    saveas(f,sprintf('/Users/abolafia/ASO_Fire/Plots/PaperPlots/Baseline_EBNFF_Qanom_timeseries_prefire_Standardized_similarDryperiod%d.png',k))
end
store
ppp
...
Baseline_Q_monthly_anoms = (Baseline_Q_Mon - nanmean(Baseline_Q_Mon(idx_prefire)))./nanstd(Baseline_Q_Mon(idx_prefire));
Realistic_Q_monthly_anoms = (Realistic_Q_Mon - nanmean(Realistic_Q_Mon(idx_prefire)))./nanstd(Realistic_Q_Mon(idx_prefire));
Obs_Q_monthly_anoms = (Obs_Q_Mon - nanmean(Obs_Q_Mon(idx_prefire)))./nanstd(Obs_Q_Mon(idx_prefire));
...
% end
saveas(f,'/Users/abolafia/ASO_Fire/Plots/PaperPlots/Baseline_EBNFF_Qanom_timeseries_prefire_monthly_Standardized.png')
...
% end
saveas(f,'/Users/abolafia/ASO_Fire/Plots/PaperPlots/EBNFF_Qanom_timeseries_postfire_monthly_Standardized.png')
