...
current_station = 'ICR'; %station closest to outlet of middle fork feather
...
%load in simulated Q - realistic adjustments 
...
post_fire_start = datenum([2021 11 30]);
pre_fire_start = datenum([2020 8 1]);
idx_postfire = find(matched_dates > post_fire_start);
idx_prefire = find(matched_dates < pre_fire_start);
idx_mid=find(matched_dates<=post_fire_start & matched_dates>=pre_fire_start);
...
xlim([-0.6 10])
ylim([-0.6 10])
[r_pre,p_pre] = corr(Baseline_Q_daily_anoms(idx_prefire) , Obs_Q_daily_anoms(idx_prefire),'type','pearson');
...
[r_post,p_post] = corr(Baseline_Q_daily_anoms(idx_postfire) , Obs_Q_daily_anoms(idx_postfire),'type','pearson');
...
[r_post,p_post] = corr(Realistic_Q_daily_anoms(idx_postfire) , Obs_Q_daily_anoms(idx_postfire),'type','pearson');
...
legend([p1 p2 p3],{'prefire baseline','post-fire baseline','post-fire realistic'},'fontsize',24,'location','northeast')

saveas(f,'/Users/abolafia/ASO_Fire/Plots/PaperPlots/Baseline_and_Realistic_EBNFF_Qanom_scatter.png')
...
saveas(f,'/Users/abolafia/ASO_Fire/Plots/PaperPlots/Baseline_and_Realistic_EBNFF_Bias_Boxplot.png')
...
%report R:
[R,p] = corr(prefire_obs,prefire_baseline,'type','pearson');
%report NSE:
NSE =nashsutcliffe([dates_prefire,prefire_baseline], [dates_prefire,prefire_obs]);
%report bias:
bias =nanmean(prefire_baseline - prefire_obs);
if p>0.001
    text(dates_prefire(1000),10,sprintf('R^{2}=%.2f (p=%.2f); NSE = %.2f; bias - %.2f',round(R^2,2),round(p,2),round(NSE,2),round(bias,2)),'fontsize',24,'color','b')
else
    text(dates_prefire(1000),10,sprintf('R^{2}=%.2f (p<0.001); NSE = %.2f; bias = %.2f',round(R^2,2),round(NSE,2),round(bias,2)),'fontsize',24,'color','b')
end
saveas(f,'/Users/abolafia/ASO_Fire/Plots/PaperPlots/Baseline_EBNFF_Qanom_timeseries_prefire.png')
...
if p>0.001
    text(dates_postfire(125),1.5,sprintf('R^{2}=%.2f(p=%.2f);NSE =%.2f;bias=%.2f',round(R_baseline^2,2),round(p_baseline,2),round(NSE_baseline,2),round(bias_baseline,2)),'fontsize',24,'color','b')
else
    text(dates_postfire(125),1.5,sprintf('R^{2}=%.2f(p<0.001);NSE =%.2f;bias=%.2f',round(R_baseline^2,2),round(NSE_baseline,2),round(bias_baseline,2)),'fontsize',24,'color','b')
end
...
if p>0.001
    text(dates_postfire(125),1,sprintf('R^{2}=%.2f(p=%.2f);NSE =%.2f;bias=%.2f',round(R_realistic^2,2),round(p_realistic,2),round(NSE_realistic,2),round(bias_realistic,2)),'fontsize',24,'color','r')
else
    text(dates_postfire(125),1,sprintf('R^{2}=%.2f(p<0.001);NSE =%.2f;bias=%.2f',round(R_realistic^2,2),round(NSE_realistic,2),round(bias_realistic,2)),'fontsize',24,'color','r')
end
xtickangle(20)
saveas(f,'/Users/abolafia/ASO_Fire/Plots/PaperPlots/EBNFF_Qanom_timeseries_postfire.png')
...
for j=1:length(idx1)-1
    ...
    store_stats = [store_stats;Mean_obs,current_R2,current_NSE,current_bias];
end
...
idx_prefire = find(matched_dates_monthly < pre_fire_start);
...
bias =nanmean(prefire_baseline - prefire_obs);
if p>0.001
    text(dates_prefire(50),6,sprintf('R^{2}=%.2f (p=%.2f); NSE = %.2f; bias - %.2f',round(R^2,2),round(p,2),round(NSE,2),round(bias,2)),'fontsize',24,'color','b')
else
    text(dates_prefire(50),6,sprintf('R^{2}=%.2f (p<0.001); NSE = %.2f; bias = %.2f',round(R^2,2),round(NSE,2),round(bias,2)),'fontsize',24,'color','b')
end
saveas(f,'/Users/abolafia/ASO_Fire/Plots/PaperPlots/Baseline_EBNFF_Qanom_timeseries_prefire_monthly.png')
...
bias_baseline = nanmean(postfire_baseline - postfire_obs);
if p>0.001
    text(dates_postfire(7),0.2,sprintf('R^{2}=%.2f (p=%.2f); NSE = %.2f; bias = %.2f',round(R_baseline^2,2),round(p_baseline,2),round(NSE_baseline,2),round(bias_baseline,2)),'fontsize',20,'color','b')
else
    text(dates_postfire(7),0.2,sprintf('R^{2}=%.2f (p<0.001); NSE = %.2f; bias = %.2f',round(R_baseline^2,2),round(NSE_baseline,2),round(bias_baseline,2)),'fontsize',20,'color','b')
end
...
bias_realistic = nanmean(postfire_realistic - postfire_obs);
if p>0.001
    text(dates_postfire(7),0.0,sprintf('R^{2}=%.2f (p=%.2f); NSE = %.2f; bias = %.2f',round(R_realistic^2,2),round(p_realistic,2),round(NSE_realistic,2),round(bias_realistic,2)),'fontsize',20,'color','r')
else
    text(dates_postfire(7),0.0,sprintf('R^{2}=%.2f (p<0.001); NSE = %.2f; bias = %.2f',round(R_realistic^2,2),round(NSE_realistic,2),round(bias_realistic,2)),'fontsize',20,'color','r')
end
saveas(f,'/Users/abolafia/ASO_Fire/Plots/PaperPlots/EBNFF_Qanom_timeseries_postfire_monthly.png')